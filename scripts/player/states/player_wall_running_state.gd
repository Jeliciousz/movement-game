...
func update_physics(delta: float) -> void:
	...
	if Time.get_ticks_msec() - shared_vars[&"wallrun_timestamp"] > _player.wallrun_duration:
		...
		if _player.velocity.dot(_player.up_direction) < 0:
			_player.velocity.move_toward(_player.velocity - _player.up_direction * _player.velocity.dot(_player.up_direction), _player.wallrun_downwards_friction * delta)
		else:
			_player.velocity.move_toward(_player.velocity - _player.up_direction * _player.velocity.dot(_player.up_direction), _player.wallrun_upwards_friction * delta)
